        ...
        <div class="row">
            <div class="col-6">
                <div class="row">
                    <div class="col-3">
                        <span class="avatar avatar-xl rounded-circle"
                            style="background-image: url(@if ($current_image == null) 'https://ui-avatars.com/api/?name={{ $username }}&background=DE6060&color=fff&rounded=true&size=112' @else {{ asset('storage/' . $current_image) }} @endif)"></span>
                    </div>
                    <div class="col-9">
                        <div class="row">
                            <div class="username col-12">
                                ...
                                @if ($show)
                                    <a href="#" data-bs-toggle="modal" data-bs-target="#profileModal"><span class="edit">Edit profile</span></a>
                                @endif
                            </div>
                        </div>
                        <div>
                            <h3 class="text-secondary">
                                @if ($user->credential == null)
                                    @if ($show)
                                        <a href="" data-bs-toggle="modal" data-bs-target="#profileModal"></a>
                                    @endif
                                @else
                                    {{ $user->credential }}
                                @endif
                                ...
                                @if (!$followed)
                                    <button class="btn btn-pill btn-primary btn-sm" wire:click="follow">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="me-1 icon icon-tabler icon-tabler-user-plus" width="24"
                ...
                <div class="mt-4">
                    <div class="row text-center">
                        <div class="col-3">
                            <a href=""
                                class="@if ($answers) text-danger @else text-secondary @endif "
                                wire:click.prevent="showAnswers">{{ $total_answers }} Answers</a>
                        </div>
                        <div class="col-3">
                            <a href=""
                                class="@if ($questions) text-danger @else text-secondary @endif"
                                wire:click.prevent="showQuestions">{{ $total_questions }} Questions</a>
                        </div>
                        <div class="col-3">
                            <a href=""
                                class="@if ($user_followers) text-danger @else text-secondary @endif"
                                wire:click.prevent="showFollowers">{{ $total_followers }} Followers</a>
                        </div>
                        <div class="col-3">
                            <a href=""
                                class="@if ($user_following) text-danger @else text-secondary @endif"
                                wire:click.prevent="showFollowing">{{ $total_following }} Following</a>
                        </div>
                    </div>
                </div>
                <hr class="mt-3 mb-3">
                ...
                @if ($questions)
                    @forelse ($questions as $question)
                        <div>
                            <div>
                                <a href="{{ route('question.index', $question->title_slug) }}"
                                    class="text-dark"><b>{{ $question->title }}</b></a>
                            </div>
                            <hr class="mt-3 mb-3">
                        </div>
                    @empty
                            ...
                            <div class="row">
                                <div class="col-1">
                                    <span class="avatar avatar-sm rounded-circle"
                                        style="background-image: url(@if ($u_f->image == null) 'https://ui-avatars.com/api/?name={{ $u_f->username }}&background=DE6060&color=fff&rounded=true&size=112' @else {{ asset('storage/' . $u_f->image) }} @endif)"></span>
                                </div>
            ...
            <div class="col-4">
                <div>
                    ...
                    <div>
                        @if ($user->employment()->exists() || $show)
                            <svg xmlns="http://www.w3.org/2000/svg"
                                class="me-1 icon icon-tabler icon-tabler-briefcase" width="24" height="24"
                                viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                <path
                    ...
                    <div class="mt-3">
                        ...
                        @endif
                        @if (!$user->education()->exists() && $show)
                            <a href="" data-bs-toggle="modal" data-bs-target="#educationModal"></a>
                ...
                <div class="mt-4">
                    ...
                    @if ($show)
                        <span class="float-end">
                            <svg role="button" data-bs-toggle="modal" data-bs-target="#topicModal"
                    ...
                    @if ($show && $followed_topics->isEmpty())
                        <div class="text-center">
                            ...
                            <button class="btn btn-outline-primary btn-pill" data-bs-toggle="modal"
                        ...
                        @foreach ($followed_topics as $topic)
                            <div class="mb-3">
                                <div class="row">
                                    <div class="col-1">
                                        <span class="avatar avatar-sm rounded-circle"
                                            style="background-image: url('https://ui-avatars.com/api/?name={{ $topic->name }}&background=DE6060&color=fff&rounded=true&size=112')"></span>
                                    </div>
                                    <div class="col-11 mt-1">
                                ...
                                <div class="col-2">
                                    @if ($image)
                                        <span class="avatar avatar-lg rounded-circle"
                                            style="background-image: url({{ $image->temporaryUrl() }})"></span>
                                    @else
                                        <span class="avatar avatar-lg rounded-circle"
                                            style="background-image: url(@if ($current_image == null) 'https://ui-avatars.com/api/?name={{ $username }}&background=DE6060&color=fff&rounded=true&size=112' @else {{ asset('storage/' . $current_image) }} @endif)"></span>
                                    @endif
